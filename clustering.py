...
class TravelRecommendationEngine:
    def __init__(self, data_path: str = "df_cluster.csv"):
        """
        Khởi tạo recommendation engine
        """
        self.df = pd.read_csv(data_path)
        self.scaler = StandardScaler()
        self.features = ['avgtemp_c', 'maxwind_kph', 'avghumidity', 'avgvis_km']
        ...
        # Lấy centroids
        self.centroids = self.df[self.df['is_centroids'] == 1].copy()
    ...
    def find_best_cluster(self, preferences: Dict) -> int:
        ...
        """
        # Tạo vector từ preferences
        user_vector = np.array([
            preferences['avgtemp_c'],
            preferences['maxwind_kph'], 
            preferences['avghumidity'],
            preferences['avgvis_km']
        ]).reshape(1, -1)
    ...
    def get_recommendations(self, preferences: Dict, top_k: int = 5) -> List[Dict]:
        ...
        for _, row in recommendations.iterrows():
            result.append({
                ...
                'lon': row['lon'],
                'month': int(row['month']),
                'avgtemp_c': float(row['avgtemp_c']),
                'maxwind_kph': float(row['maxwind_kph']),
                'avghumidity': float(row['avghumidity']),
                'avgvis_km': float(row['avgvis_km']),
    ...
    def get_cluster_info(self, cluster_id: int) -> Dict:
        ...
        return {
            ...
            'avg_visibility': float(cluster_data['avgvis_km'].mean()),
            'centroid_location': {
                'city': str(centroid.iloc[0]['city']) if len(centroid) > 0 else 'Unknown',
    ...
    def search_by_location(self, location_name: str, top_k: int = 5) -> List[Dict]:
        ...
        # Tìm trong city và province
        mask = (
            self.df['city'].str.lower().str.contains(location_name, na=False) |
            self.df['province'].str.lower().str.contains(location_name, na=False)
        )
        
        results = self.df[mask].sort_values('score', ascending=False).head(top_k)
